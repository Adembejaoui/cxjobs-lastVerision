...
export function setup() {
  const res = http.get(`${BASE_URL}/api/job-offers?limit=100`, {
  ...
  console.log(`Setup: Found ${jobOfferIds.length} job offers`);
  return { jobOfferIds };
}
...
function textSummary(data, options = {}) {
  ...
  if (data.metrics.checks) {
    ...
    Object.entries(data.metrics.checks.values).forEach(([checkName, checkData]) => {
      const total = checkData.passes + checkData.fails;
      if (total > 0) {
        const passRate = (checkData.passes / total * 100).toFixed(2);
        const status = checkData.passes > checkData.fails ? color('✓') : red('✗');
        output += `${indent}  ${status} ${checkName}: ${passRate}% (${checkData.passes}/${total})\n`;
      }
    });
  }
