...
export function CoverflowCarousel({
  ...
  /** Scheduled paint during drag to batch updates */
  const paintRafRef = React.useRef<number | null>(null);
  ...
  const paint = React.useCallback(() => {
    ...
    cardRefs.current.forEach((card, index) => {
      ...
      // PERF: Skip cards that are far away (beyond 3 cards from center)
      // They're nearly invisible anyway due to fade/opacity
      if (distance > 3) {
  ...
  /** Batched paint for drag operations - uses rAF to avoid blocking main thread */
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Use batched paint during drag for smooth 60fps
    schedulePaint();
  };
    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-10 outline-none ring-ring focus-visible:ring-2 active:cursor-grabbing touch-none"
          style={{
            perspective: `calc(var(--cf-card) * ${perspective})`,
            // Allow both horizontal and vertical touch for natural scrolling
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-background/70 p-2 text-foreground backdrop-blur transition hover:bg-background md:left-6"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-background/70 p-2 text-foreground backdrop-blur transition hover:bg-background md:right-6"
            >
